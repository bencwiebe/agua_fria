...
def create_df_with_adaptation(data_dict, image_percentile):
    ...
    df['20th_percentile'] = df.apply(lambda row: np.percentile(row.value_array, 20), axis=1)
    df['50th_percentile'] = df.apply(lambda row: np.percentile(row.value_array, 50), axis=1)
    df['80th_percentile'] = df.apply(lambda row: np.percentile(row.value_array, 80), axis=1)
    df['95th_percentile'] = df.apply(lambda row: np.percentile(row.value_array, 95), axis=1)
...
df = create_df_with_adaptation(data_dict, 80)
